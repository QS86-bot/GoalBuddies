 ...
 *    die alleen 0240 leest, blijft groen als iemand `image/svg+xml` aan een
...
describe('de emmer en de app noemen dezelfde typen', () => {
  ...
  it('de toegestane typen komen letterlijk uit migratie 0240', () => {
    expect([...typenUit(M0240)].sort()).toEqual([...CHATDOC_TYPES].sort());
  });

  it('de bovengrens komt letterlijk uit migratie 0240', () => {
    const getal = /'chatdocs',\s*'chatdocs',\s*false,\s*(?:--[^\n]*\n|\s)*?(\d+)/.exec(M0240);
    expect(getal, 'geen grootte in de insert van 0240').not.toBeNull();
    expect(Number(getal?.[1])).toBe(CHATDOC_MAX_BYTES);
  });

  it('de emmer is privé', () => {
    expect(M0240).toMatch(/values\s*\(\s*\n?\s*'chatdocs',\s*\n?\s*'chatdocs',\s*\n?\s*false/);
  });
...
describe('geen enkel type dat een browser kan uitvoeren', () => {
  ...
  it.each(NOOIT)('%s staat niet in de emmer — %s', (type) => {
    expect(typenUit(M0240)).not.toContain(type);
  });
...
describe('het pad ligt vast en de naam niet', () => {
  it('een gebouwd pad eindigt altijd op .pdf, wat de gebruiker ook koos', () => {
    ...
    //    haalt de CHECK van 0242 dus nooit, en het pad is per constructie in
...
describe('schoneBestandsnaam maakt precies wat de CHECK toelaat', () => {
   ...
   *    `chat_messages_attachment_name_vorm` (0242) moeten dezelfde tekens
   ...
   */
  it('de tekenklasse in de app is die van de CHECK in 0242', () => {
...
describe('soort, extensie en emmer zijn één drieklank', () => {
  /**
   * ⚠️⚠️ **De naad tussen de CHECK en `soortBijlage()`.** De CHECK van 0242 paart
  ...
  it('de fotoextensies van de app staan alle drie in de fototak van de CHECK', () => {
    ...
    expect(tak, 'geen fototak in 0242').not.toBeNull();
