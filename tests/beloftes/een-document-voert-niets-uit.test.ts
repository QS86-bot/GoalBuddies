...
const M0239 = readFileSync('supabase/migrations/0239_een_document_hoort_bij_een_groep.sql', 'utf8');
const M0236 = readFileSync(
  'supabase/migrations/0241_een_bijlage_zegt_welke_soort_hij_is.sql',
  'utf8',
);
 ...
 *    die alleen 0239 leest, blijft groen als iemand `image/svg+xml` aan een
...
describe('de emmer en de app noemen dezelfde typen', () => {
  ...
  it('de toegestane typen komen letterlijk uit migratie 0239', () => {
    expect([...typenUit(M0239)].sort()).toEqual([...CHATDOC_TYPES].sort());
  });

  it('de bovengrens komt letterlijk uit migratie 0239', () => {
    const getal = /'chatdocs',\s*'chatdocs',\s*false,\s*(?:--[^\n]*\n|\s)*?(\d+)/.exec(M0239);
    expect(getal, 'geen grootte in de insert van 0239').not.toBeNull();
    expect(Number(getal?.[1])).toBe(CHATDOC_MAX_BYTES);
  });

  it('de emmer is privé', () => {
    expect(M0239).toMatch(/values\s*\(\s*\n?\s*'chatdocs',\s*\n?\s*'chatdocs',\s*\n?\s*false/);
  });
...
describe('geen enkel type dat een browser kan uitvoeren', () => {
  ...
  it.each(NOOIT)('%s staat niet in de emmer — %s', (type) => {
    expect(typenUit(M0239)).not.toContain(type);
  });
...
describe('het pad ligt vast en de naam niet', () => {
  it('een gebouwd pad eindigt altijd op .pdf, wat de gebruiker ook koos', () => {
    ...
    //    haalt de CHECK van 0241 dus nooit, en het pad is per constructie in
...
describe('schoneBestandsnaam maakt precies wat de CHECK toelaat', () => {
   ...
   *    `chat_messages_attachment_name_vorm` (0241) moeten dezelfde tekens
   ...
   */
  it('de tekenklasse in de app is die van de CHECK in 0241', () => {
    const uitMigratie = /attachment_name\s*!~\s*'(\[[^']+\])'/.exec(M0236);
    expect(uitMigratie, 'geen tekenklasse in 0241').not.toBeNull();
...
describe('soort, extensie en emmer zijn één drieklank', () => {
  /**
   * ⚠️⚠️ **De naad tussen de CHECK en `soortBijlage()`.** De CHECK van 0241 paart
  ...
  it('de fotoextensies van de app staan alle drie in de fototak van de CHECK', () => {
    ...
    expect(tak, 'geen fototak in 0241').not.toBeNull();
