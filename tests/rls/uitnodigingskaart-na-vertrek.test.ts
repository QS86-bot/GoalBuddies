...
describe.skipIf(!rlsTestsConfigured)('de uitnodigingskaart na een vertrek', () => {
  ...
  beforeAll(async () => {
    ...
    // Weggestuurd — en met opzet níet geblokkeerd, zie IJKING B.
    psql(
      `insert into group_members (group_id, user_id, role, status) ` +
        `values ('${groupId}', '${weggestuurd.id}', 'member', 'inactive') ` +
        `on conflict (group_id, user_id) do update set status = 'inactive'`,
    );
  ...
  describe('maar wie eruit is, kijkt niet meer naar binnen', () => {
    ...
    it(
      'laat een weggestuurd lid de uitnodigingsteller niet volstoken',
      async () => {
        /**
