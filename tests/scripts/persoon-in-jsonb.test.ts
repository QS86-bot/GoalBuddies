...
describe('de lijst met rechtgezette vondsten', () => {
  ...
  it('noemt bij elke regel de migratie die hem rechtzette', () => {
    expect(RECHTGEZET.length).toBeGreaterThan(0);
  ...
  it('laat alleen precies die combinatie door', () => {
    const bekend = RECHTGEZET[0];
    if (bekend === undefined) throw new Error('RECHTGEZET is leeg');

    expect(isRechtgezet(bekend.bestand, bekend.sleutel)).toBe(true);
    expect(isRechtgezet(bekend.bestand, 'iets_anders_by')).toBe(false);
    expect(isRechtgezet('0001_schema.sql', bekend.sleutel)).toBe(false);
  });
});
