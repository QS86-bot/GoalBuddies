...
describe('de opstelling zelf', () => {
  it('draait de suite én de basis', () => {
    expect(STAPPEN.map((s: { commando: string }) => s.commando)).toEqual(['typecheck', 'lint', 'test']);
  });

  // ⚠️ Vandaag is er geen controle die een database nodig heeft: er is nog geen
  //    schema. Komt de eerste (een RLS-suite, een kolomrechtencontrole), zet
  //    hem dan hier bij naam neer — anders telt "geen database" als groen.
  it('kent de database-afhankelijke controles bij naam', () => {
    expect(HEEFT_DATABASE_NODIG).toBeInstanceOf(Set);
