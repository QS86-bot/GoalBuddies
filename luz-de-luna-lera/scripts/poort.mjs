...
export const STAPPEN = [
  ...
  // ⚠️ Zodra er een schema is, hoort hier de RLS-suite bij — als stap met
  //    `database: true`, zodat "geen database" ongemeten telt en niet groen.
  //    Zie docs/LESSEN-UIT-GOALBUDDIES.md, les over de ongemeten poort.
];
...
export const HEEFT_DATABASE_NODIG = new Set([
  // ⚠️ Leeg tot de eerste controle die `pg_get_functiondef()` of het echte
  //    project leest. Zet hem hier bij naam neer op de dag dat je hem schrijft,
  //    anders meldt de poort "alles groen" over iets dat niets gemeten heeft.
]);
  ...
  if (ongemeten.length > 0) {
    process.stdout.write(
      `· ${ongemeten.length} controle(s) zonder database: ${ongemeten.map((u) => u.naam).join(', ')}.\n` +
        '  Die hebben niets gemeten. Zet de databaseopstelling of de sleutels klaar en draai opnieuw.\n\n',
    );
  }
