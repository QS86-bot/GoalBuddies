...
| 34 | **Een document in de groepschat** | `chatdocs`-bucket, `chat_messages.attachment_url` + `attachment_name`, `groepschat()`, `app/groep/chat/[id].tsx` | Aan een groepslid: het document dat een ander lid in **deze** groep plaatste, met de naam die de plaatser koos. Aan elke andere groep, ook een die een lid met je deelt: niets | ⚠️ **Toegevoegd 10-09-2026 (QS8-72, migraties 0235 t/m 0238).** Zelfde padvorm, zelfde vier policies en dezelfde twee tellers als rij 32 — en juist daarom staat het risico hier ergens anders. **Kan hieruit iemands gemiste week worden afgeleid?** Nee: net als bij rij 32 en rij 8 is dit wat iemand zélf plaatst, en niets in dit oppervlak varieert op status, reeks of week. **Kan iemand het buiten de UI om uitlezen?** Nee — het pad draagt de groep (`<group_id>/<sender_id>/…`), `mag_groep_lezen()` leest en `is_group_member()` schrijft, en zonder `chatdocs_select` valt er niets te ondertekenen. ⚠️⚠️ **Wat hier wél nieuw is: het veiligheidsargument van rij 32 geldt níét.** §6 van het fotodossier zegt dat willekeurige bytes niet erg zijn omdat ze *"in een `<Image>` belanden — willekeurige bytes renderen niet en voeren niets uit"*. Een document belandt niet in een `<Image>`; het gaat naar de systeembrowser, en een `text/html` vanaf de storage-origin ís daar uitvoerbare code met leesrecht op alles wat die origin bewaart. De emmer toetst alleen de **gedeclareerde** header, dus de grendel is `allowed_mime_types` — plus het feit dat `uploadChatdoc()` `contentType` **hard** declareert en niet uit de kiezer overneemt. ⚠️⚠️ **En dat is meervoud: niet de allowlist van deze emmer maar die van álle vier.** 📏 Gemeten op 10-09-2026 en zelf nageverifieerd: een lid verhuist zijn eigen object met één `update` van `chatfotos` of `avatars` naar `chatdocs` — policies worden over emmers heen ge-OR'd en de database kijkt daarbij niet terug naar het type. Het effectieve typebereik is dus de unie, vandaag {pdf, jpeg, png, webp} en alle vier inert, dus er staat niets open; wat er wél openstond is de aanname, en die zat in drie documenten. De denylist leest sinds 10-09 élke emmer in de migratiemap; de verhuizing zelf is QS8-407. Acht typen staan met reden op een denylist in `tests/beloftes/een-document-voert-niets-uit.test.ts`; die is er omdat een gelijkheidstoets tussen app en migratie het geval *"iemand voegt `image/svg+xml` aan allebei toe"* per definitie niet vindt. ⚠️⚠️ **En er komt gebruikerstekst bij die de groep te zien krijgt: `attachment_name`.** Dat is nieuw ten opzichte van rij 32 en 33, waar alleen een beeld overging. De naam is geen tegenslag, maar hij is wél een leugenkanaal: met een RLO (U+202E) erin rendert `verslag<RLO>fdp.exe` als `verslagexe.pdf`, terwijl het pad `.pdf` zegt. De CHECK `chat_messages_attachment_name_vorm` weigert die tekens, `schoneBestandsnaam()` haalt ze er vooraf uit, en de UI leest de sóórt uit het **pad** (`soortUitPad()`) en nooit uit de naam. ⚠️ **De soort is aan de extensie gepaard** (`chat_messages_attachment_eigen_pad`): een `photo`-rij kán niet naar een `.pdf` wijzen en andersom. Zonder die paring breekt er niets zichtbaars als ze uit elkaar lopen — je tekent tegen de verkeerde emmer en de bubbel zegt "niet meer beschikbaar". ⚠️ **Bij accountverwijdering gaat het document weg én de naam mee** — `jaarrekening-jansen.pdf` is zelf een persoonsgegeven. De functie heet daarom `wis_bijlagen_van_vertrekker()` en dekt beide chat-emmers. Getest in `tests/rls/chatdocbucket.test.ts` (negentien gevallen, zeven grendels apart met de hand rood gemaakt), `tests/rls/een-document-is-wat-het-zegt.test.ts` (negentien, vier grendels) en `tests/beloftes/een-document-voert-niets-uit.test.ts` (zevenveertig). Onderbouwing in `docs/decisions/2026-09-10-een-document-is-geen-foto.md` |
...
| 34 | Een document in de groepschat | Niets. Een document is wat iemand zélf plaatst — oppervlak 8 en niet 3 — en er valt in een open groep niets extra's te openen. Identiek aan rij 32, en om dezelfde reden | ✅ n.v.t. |
