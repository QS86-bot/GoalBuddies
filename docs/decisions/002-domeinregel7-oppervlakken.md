...
| 31 | **De straf bij een uitstelverzoek** | `commitments`, `commitments_select` vierde tak, `gevraagd_om_uitstel_op()`, `app/groep/[id].tsx`, `app/doel/[id].tsx` | Aan de groep die om uitstel gevráágd is: de straffen op dát doel, vanaf `set`. Aan elke andere groep: niets | ⚠️ **Toegevoegd 09-09-2026 (QS8-370, migratie 0213), en dit is een verruiming die Quinten zelf besloten heeft: *"Iedereen van de groep mag de straf zien"*.** De aanleiding is een gat en geen wens. `zet_streefdatum()` weigert sinds 0184 een straf op `set` vooruit te schuiven; `beslis_deadline_verzoek()` doet dat met opzet niet, want 0184 wijst de groepsroute zelf aan als de weg die openblijft. 📏 Gemeten: het lid dat goedkeurt zag de straf niet — `commitment_zichtbaar_voor_groep()` geeft `unlocked, due, resolved` en `commitment_zichtbaar_voor_persoon()` geeft `due, resolved`, dus bij `set` was het nul voor allebei de vormen. Het akkoord was dus blind, en dáármee werd een commitment device stilzwijgend losser gemaakt — de spiegelzijde van domeinregel 5. ⚠️ **Kan hieruit iemands gemiste week worden afgeleid?** Nee. Een straf op `set` zegt "ik heb mezelf een consequentie opgelegd", niet "ik heb iets niet gehaald"; de tegenslagkant van een straf is `due`, en die was voor de begunstigde al zichtbaar (rij 20). Wat hier bijkomt is de *afspraak*, niet het *falen*. ⚠️ **Kan iemand het buiten de UI om uitlezen?** Alleen wie er recht op heeft: de tak is een policy en geen filter in een scherm, en `gevraagd_om_uitstel_op()` is SECURITY DEFINER met `mag_groep_lezen()` — dezelfde opvatting van "mag deze groep lezen" als de tweede tak, en niet een tweede. ⚠️ **Dit verruimt `commitment_zichtbaar_voor_groep()` níet, en dat is met opzet.** Die lijst geldt voor de **begunstigde** groep, en de groep die beslist hoeft dat niet te zijn — `vraag_deadline_verschuiving()` eist alleen een koppeling via `goal_group_links`. Verruimen van die lijst zou dus tegelijk te weinig zijn (de gemeten gevallen blijven open) en te veel (élke begunstigde groep zou élke straf zien, ook zonder dat er ooit iets gevraagd is). ⚠️ **Elk verzoek telt, niet alleen een open verzoek.** Een oppervlak dat dichtklapt zodra er beslist is, neemt de beslisser het zicht af op wat hij zojuist heeft toegestaan — het tegendeel van "auditeerbaar". ⚠️ **En de eigenaar weet het vooraf**, want anders is dit een consequentie die stilzwijgend aan gaat: het aanvraagscherm zegt vóór de verzendknop dat de groep de straf gaat zien en hem daarna blijft zien (`deadline.straf_wordt_zichtbaar`). Dit is dus route 3 uit domeinregel 7 — via de gebruiker zelf — en niet een vierde. ⚠️ **Alleen `type = 'penalty'`**, en zonder statuslijst: `wordtZichtbaarBijUitstelverzoek()` in de client draagt exact dezelfde grens, want een waarschuwing die smaller is dan het oppervlak is geen waarschuwing. Getest in `tests/rls/uitstelbeslisser-ziet-de-straf.test.ts` (vijf grendels, elk apart met de hand rood gemaakt) en `tests/beloftes/uitstelbeslisser-krijgt-het-te-zien.test.ts` |
