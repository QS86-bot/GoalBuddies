...
| 32 | **Een foto in de groepschat** | `chatfotos`-bucket, `chat_messages.attachment_url`, `groepschat()`, `app/groep/chat/[id].tsx` | Aan een groepslid: de foto die een ander lid in **deze** groep plaatste. Aan elke andere groep, ook een die een lid met je deelt: niets | ⚠️ **Toegevoegd 09-09-2026 (QS8-71, migraties 0222 t/m 0226).** Dit oppervlak is niet nieuw in de zin dat de groep iets nieuws over een pérsoon te zien krijgt — het is wat iemand zelf plaatst, net als rij 8 — maar het is het eerste dat een **tweede opslagsysteem** in het spel brengt, en daar zit het risico. **Kan hieruit iemands gemiste week worden afgeleid?** Nee: een foto is wat de plaatser kiest te laten zien, en niets in dit oppervlak varieert op status, reeks of week. **Kan iemand het buiten de UI om uitlezen?** Dat is hier de scherpe vraag, want een storage-object heeft een URL die de RLS van `chat_messages` **niet** kent — het object en de rij zijn twee onafhankelijk geautoriseerde dingen die de app als één ding toont. ⚠️⚠️ **Daarom draagt het pad de groep en niet de gebruiker.** 📏 Gemeten met Alice in groep A én B, Bob alleen in A, Carol alleen in B: met de vorm van `avatars` (`<user_id>/…`, lezend op `shares_group_with_user()`) is dat predicaat wáár voor Carol — zij deelt groep B met Alice — en las zij dus de foto uit **groep A**. Dat is oppervlak 24 in een nieuwe verpakking. Het pad is `<group_id>/<sender_id>/<naam>.<ext>`; het eerste segment draagt de leesgrens, het tweede de schrijfgrens. ⚠️ **De kolomgrens is een CHECK en geen policy** (`chat_messages_attachment_eigen_pad`), want `authenticated` heeft sinds 0059 een INSERT-kolomgrant op `attachment_url` en **RLS kan geen kolommen beperken**. Zonder die CHECK zet een lid met één PostgREST-verzoek het pad van een ándere groep of een extern adres in dat veld, en dan laadt elk groepslid dat adres uit zijn eigen `<Image>`. Het is een **vormtoets en geen prefixtoets**, en dat is de derde keer: 0127 deed dit met een `like` voor `avatar_url` en 0129 moest erbovenop omdat `<mij>/../<ander>/a.png` erdoorheen liep. 📏 Zeven gevallen gemeten, zes geweigerd (ander lid, andere groep, extern adres, padtraversal, `.svg`, en een tweede URL achter een regeleinde) en één toegelaten. ⚠️ **`mag_groep_lezen()` leest en `is_group_member()` schrijft**, want `archiefleesgat()` (0153) scant óók `schemaname = 'storage'`: een archief is leesbaar en niet beschrijfbaar. ⚠️ **De ondertekende URL blijft een tweede pad en dat is aanvaard**, niet opgelost: eenmaal getekend werkt hij een uur voor iedereen die hem heeft. Dat is dezelfde klasse als een schermafdruk; de rem is de geldigheidsduur. Wat wél dicht is: een niet-lid kan er zelf nooit een tekenen. ⚠️ **Bij accountverwijdering gaat de foto weg en blijft het gesprek** — anders dan bij een bewijsfoto (QS8-391), die blijft omdat hij bewijs is vóór iemand anders. 📏 Dat was bovendien een 23514 en geen ontwerp: het `on delete set null` van 0031 maakt `sender_id` leeg en de CHECK van 0223 eist er een, dus accountverwijdering brak op een fotobericht. Derde keer dat een CHECK op deze tabel een referentiële actie blokkeert. Getest in `tests/rls/chatfotobucket.test.ts` (twintig gevallen, vier grendels apart met de hand rood gemaakt) en `tests/rls/een-foto-verlaat-zijn-groep-niet.test.ts` (vier routes, elk met een must-allow en een must-deny) |
| 33 | **Een foto als bewijs bij een voltooiing** | `bewijsfotos`-bucket, `completions.attachment_url`, `openstaande_beoordelingen()`, `app/beoordelen.tsx` | Aan wie de vóltooiing mag lezen: de eigenaar en een groepsgenoot via `shares_group_with_goal()`. Aan een lid van een ándere groep die je deelt: niets. Bij een doel zónder groep: aan niemand behalve de eigenaar | ⚠️ **Toegevoegd 09-09-2026 (QS8-391, migraties 0227 t/m 0231).** Zelfde tweede-opslagsysteem als rij 32, en dezelfde naad — het object en de rij zijn onafhankelijk geautoriseerd. ⚠️⚠️ **Maar er zit hier een val in die bij rij 32 niet bestond, en die is gemeten.** Het pad is op het **weekdoel** gesleuteld, dus `mag_weekdoel_lezen` voelt als het goede predicaat. Het is het verkeerde: 📏 `completions_select` erft transitief het statusfilter van `weekly_goals_select` — zijn `exists` leest `weekly_goals`, en RLS geldt óók binnen een policy-subquery. Een `security definer` hulpfunctie die het predicaat *letterlijk overschreef* omzeilde die overerving en gaf een groepsgenoot het bewijs van een **gemiste week**: `shares_group_with_goal` = true, rij = 0, object = 1. Precies het schaamtemoment waar deze regel voor bestaat. ✅ De policy stelt nu de vraag (`exists (select 1 from completions where attachment_url = name)`) met een **invoker**-functie, dus de RLS van `completions` beslist en dit oppervlak beweegt vanzelf mee. **Kan hieruit een gemiste week worden afgeleid?** In een **beschermde** groep niet — het object volgt de rij op elke route, langs vier routes nagemeten. In een **open** groep wél, en dat is besluit A41 en geen lek: 📏 beschermd geeft 0/0/0 en open geeft 1/1/1 op weekdoel, voltooiing en object. ⚠️ Het object is daar nooit ruimer dan de rij — het erft dezelfde tak. **Met één API-verzoek buiten de UI om?** Nee: geen pad zonder ondertekening, en geen ondertekening zonder `bewijsfotos_select`. Onderbouwing in `docs/decisions/2026-09-09-een-foto-als-bewijs.md` §2 en §7. |
| 34 | **De rij van een gekoppeld doel** | `goals_select`, `goal_dashboard`, `app/doel/[id].tsx` | Aan elke groepsgenoot van een gekoppeld doel: de **hele rij** van `goals`. Sinds migratie 0236 alleen `id`, `owner_id`, `title`, `description`, `category`, `target_date`, `status`, `created_at`, `updated_at` en `ritme` | ⚠️⚠️ **Toegevoegd 09-09-2026 (QS8-392 en QS8-393, migratie 0236), en dát deze rij ontbrak ís de bevinding.** Dit register telde 64 oppervlakken en `goals_select` was er geen van; `identity_statement` en `max_points` kwamen er allebei geen enkele keer in voor. Daardoor zijn de twee verplichte vragen hier nooit gesteld, en zijn er kolommen op deze rij beland die er niet horen. **Kan hieruit iemands gemiste week worden afgeleid?** Tot 0236: **ja, exact.** 📏 Gemeten — `max_points` sommeert `points_ceiling` over álle weekdoelen behalve `excused`, terwijl `weekly_goals_select` juist `missed`, `carried` en `cancelled` verbergt; `points_ceiling` staat vast op 2 en is niet client-schrijfbaar, dus het verschil ís het aantal gemiste weken: `max_points=8, zichtbaar=2 → 3 verborgen weken`. Een adempauze geeft géén gat, dus het signaal wijst uitsluitend op tegenslag; bij dagelijks pollen is het bovendien niet alleen hoevéél maar wélke week. ⚠️ Dat is een ándere klasse dan wat §4 accepteert: dáár is de afwezigheid van een weekdoel in een cyclus aanvaard als **zwak** signaal, met als reden dat die persoon misschien niets gepland had — en `max_points` haalt precies die dubbelzinnigheid weg. **Kan iemand het buiten de UI om uitlezen?** Ja, het was één `GET /rest/v1/goals?select=max_points`. ⚠️⚠️ **En er zat een tweede lek op dezelfde rij dat geen tegenslag is maar zwaarder:** `identity_statement` draagt het antwoord op de vraag wie je wordt als dit lukt, en boven dat invoerveld staat `coach.alleen_voor_jou`, dat zegt dat je groep het nooit ziet. 📏 Gemeten met een echt verzoek: een groepsgenoot in een **beschermde** groep las de zin woordelijk terug. Bij die vraag schrijven mensen op waar ze vandaan komen — verslaving, scheiding, ziekte. **Hoe het dicht is:** RLS kan geen kolommen beperken, dus een kolomgrant. ⚠️ Een `revoke select (kolom)` werkt níet als er een tabelbrede grant onder ligt (📏 `authenticated=rx`); de enige vorm die werkt is het tabelrecht intrekken en per kolom teruggeven. De eigenaar leest zijn drie kolommen uit de definer-view `mijn_doelvelden`, de vorm van `mijn_profiel`. Meegenomen omdat ze dezelfde klasse zijn en geen lezer hebben: `beoordelaar_weggehaald_op` en `losgekoppeld_op`. ⚠️⚠️ **De valstrik van de reparatie staat hier omdat de volgende lezer hem tegenkomt:** `goal_dashboard` moet `security_invoker = true` blijven. `weekly_total` klopt per kijker (📏 eigenaar 2, groepsgenoot 1) juist omdát de subquery als de aanroeper draait; wie die view op definer zet om de kolommen te kunnen maskeren, maakt van `weekly_total` **een tweede exemplaar van precies dit lek**. Getest in `tests/rls/doelkolommen.test.ts` — zes grendels, elk apart met de hand rood gemaakt, en mutatie D is die valstrik |
...
| **"Doel afgerond" en "inzet verschuldigd" in dezelfde feed** | `meld_doel_af()` plaatst in élke gekoppelde groep, `meld_commitment()` in de begunstigde groep | Ja, maar smal: alleen in een groep die zowel gekoppeld **als** begunstigde is | ⚠️ **Nieuw sinds QS8-322/0238 (08-09-2026), en gevonden door de security-ronde en niet door de bouwer.** Vóór 0238 sloten deze twee elkaar uit: afronden annuleerde de straf, dus een `commitment_due` betekende altijd "niet afgerond". Sinds 0238 blijft de straf staan bij een late afronding, en dan verschijnen ze samen — waaruit een lid kan afleiden dat de deadline gemist is. Dat is een **verruiming** en geen bestaand lek: precies deze combinatie was er eerder niet. Ze valt onder de benoemde uitzondering van domeinregel 7 (*een straf die de gebruiker zelf vooraf heeft ingesteld en bevestigd*), en `commitment_due` zégt op zichzelf al dat een deadline verstreken is. Wat er nieuw bij komt is dat de groep nu ook ziet dat er ondanks die straf afgerond is. **Bewust geaccepteerd**, want het alternatief is het systeembericht van de straf onderdrukken zodra er ook afgerond is, en dan is een straf stilzwijgend geworden — precies wat domeinregel 5 verbiedt. ⚠️ **Beide standen**: dit staat los van `groups.zichtbaarheid`, want het is de uitzondering en niet de open-groep-verruiming |
...
| 33 | Een foto als bewijs bij een voltooiing | **De bewijsfoto bij een gemiste week.** ⚠️⚠️ **Deze rij zei tot 09-09-2026 het tegenovergestelde, en dat was onjuist.** Er stond: *"`completions_select` heeft géén open-groepstak, dus een open groep krijgt hier niets extra's."* Dat is dezelfde denkfout als de `security definer`-versie die 0227 §3 net repareerde, één laag hoger: `completions_select` heeft die tak niet **in zijn eigen tekst**, maar zijn `exists` leest `weekly_goals` — en RLS geldt óók binnen een policy-subquery. Hij erft `deelt_open_groep_met_doel()` net zo goed als hij het statusfilter erft. 📏 Gemeten met dezelfde kijker, hetzelfde doel en dezelfde **gemiste** week, en alleen `zichtbaarheid` omgezet: beschermd → weekdoel 0, voltooiing 0, object 0; open → 1, 1, **1**. ✅ Het gedrag zelf is juist en volgt A41 — een open groep ziet tegenslag, dat is de hele keuze — en het object volgt de rij exact, in beide standen. Wat er fout was, was deze aantekening: een geërfde verruiming die nergens als besluit stond. ⚠️ `zichtbaarheid:controle` kán dit niet vinden: die zoekt tekstueel naar `open_groep` of `zichtbaarheid`, en `bewijsfotos_select` noemt geen van beide — hij erft ze. `tests/rls/een-bewijsfoto-volgt-zijn-voltooiing.test.ts` draagt sinds 09-09 een must-allow én een must-deny op precies dit geval. |
| 34 | De rij van een gekoppeld doel | Niets. De kolomgrant op `goals` geldt voor de rol `authenticated` en niet per groep, dus een open groep krijgt hier precies evenveel als een beschermde. ⚠️ Dat is met opzet: `identity_statement` is een antwoord aan de Doelcoach en niet aan de groep, en `max_points` rekent over verborgen rijen — dat laatste is in een **open** groep even ongewenst, want A41 opent tegenslag die de gebruiker zelf deelt en niet een teller waaruit je hem terugrekent. Wie dit ooit aan `groups.zichtbaarheid` wil koppelen, leest eerst rij 34 in de hoofdtabel: de reparatie is een kolomgrant, en een grant kent geen groepen | ✅ n.v.t. |
