...
| 32 | **Een gedeelde taak uit De Lijst** | `todo_items`, `zet_taakzichtbaarheid()`, `app/(tabs)/lijst.tsx` | Aan de **ene groep die de eigenaar per taak aanwijst**: die taak — de tekst, of hij af is, en wanneer. Aan elke andere groep, ook een andere groep van dezelfde eigenaar: niets | ⚠️ **Toegevoegd 09-09-2026 (QS8-381, migratie 0220).** Besluit van Quinten: delen gaat **per taak** (variant B), en bewust niet per lijst — wat de wens letterlijk vroeg. ⚠️⚠️ **Dat verschil is de hele reden dat domeinregel 7 hier niet verruimd hoeft te worden.** Bij "de hele lijst met één schakelaar" ziet de groep ook wat er blijft liggen, en twaalf open regels waarvan er drie al twee weken staan is de achterstand van een ander — precies het schaamtemoment waar de Habit Huddle-analyse voor waarschuwt. Bij delen per taak ziet de groep alleen wat de eigenaar heeft aangewezen, en dat is dezelfde vorm als de drie routes die de regel al toelaat: ze lopen alle drie via de gebruiker zelf. Er komt dus **geen derde benoemde verruiming** naast A15 en A7. **Kan hieruit iemands gemiste week worden afgeleid?** Nee, en dat is een eigenschap van het object en niet van een `where`-regel: een taak draagt geen cyclus, geen punten, geen reeks en geen goedkeuring (domeinregel 9 en 10, en zo staat het in de tabelcomment van 0219). Er ís geen week om te missen. Wat de groep wél kan zien is dat een gedeelde taak nog niet af is — maar dat is een regel die de eigenaar zelf heeft aangewezen, en hij haalt hem met dezelfde knop weer weg. **Kan iemand het buiten de UI om uitlezen?** Alleen wie er recht op heeft. `todo_items_select` eist drie dingen tegelijk: `visibility = 'group'`, een niet-lege `shared_group_id`, en `mag_groep_lezen(shared_group_id)`. 📏 Gemeten met echte JWT's tegen PostgREST: een lid van een ándere groep van dezelfde eigenaar krijgt nul rijen, en iemand buiten elke groep ook. ⚠️⚠️ **De keuze van de gróep is het eigenlijke besluit van deze rij.** "Openbaar voor je groep" zegt niet welke, en een gebruiker kan er in meer dan één zitten — onwrikbare regel 18 vraag 6 in zijn zuiverste vorm. Eén vinkje dat élke groep bereikt, is precies het geval waar de regel voor bestaat: *zit er een leidinggevende in de groep, dan beschermt de regel niet tegen schaamte maar tegen een beoordelingsgesprek.* De taak draagt daarom de groep waarmee hij gedeeld is. 📏 Het scherm volgt `beslissendeGroep()` uit QS8-56: bij één groep vraagt de app niets, pas bij twee of meer verschijnt de keuze. ⚠️ **RLS kan geen kolommen beperken, en dat is hier besloten en niet ontsnapt:** een groepsgenoot leest de héle rij, dus ook `done_at`, `created_at` en `order_index`. Dat is gewenst — *"ik zei dat ik dit zou doen, en het is af"* is precies wat delen oplevert — en er zit geen oordeel bij. Wie ooit een kolom wíl verbergen, heeft een view of een kolomgrant nodig en niet een extra conjunct. ⚠️ **Het schrijfpad is een RPC en geen kolomgrant.** `visibility` en `shared_group_id` staan in geen enkele grant, en `pin_taak()` weigert ze bovendien; `zet_taakzichtbaarheid()` komt er langs met een sessiesleutel die het **taak-id** draagt en niet een vlag, zodat hij precies één rij doorlaat. ⚠️ **En het gaat beide kanten op.** 📏 Bij `daily_moves` ging dat mis: 0197 trok UPDATE en DELETE in, en sindsdien is de zichtbaarheid van een Dagzet onveranderlijk. Een lijst die je openbaar maakt en nooit meer prive kunt zetten, is geen keuze maar een val. ⚠️⚠️ **Het delen eindigt met het lidmaatschap eronder, en dat is een gerepareerd lek.** 📏 Gemeten in de security-ronde: `verwijder_lid()` gaf `{"ok": true}`, het lidmaatschap werd `inactive`, en de beheerder las de taak van het ex-lid daarna gewoon terug — terwijl diezelfde RPC een openstaand `deadline_requests` wél intrekt en de `goal_group_links` wél weghaalt. De Lijst stond niet in die opruiming, want die functies zijn ouder dan zij. De grendel hangt nu aan het **feit** en niet aan de twee functies: de trigger `group_members_taken_sluiten` op `group_members` (`after delete or update`). Dat is dezelfde vorm en dezelfde reden als rij 30 hieronder — *een beheerder kan met één kaal verzoek `status = 'inactive'` zetten* — mét een tweede reden erbij die hier gemeten is: de routes eindigen verschillend, want `verlaat_groep()` **verwijdert** de rij waar `verwijder_lid()` hem op `inactive` zet. ⚠️ **Drie randen die gemeten zijn.** Verdwijnt de groep, dan zet de foreign key `shared_group_id` op `null`, normaliseert `pin_taak()` de rij terug naar `private` en valt de taak terug op eigenaar-only — de faalstand sluit, en `('group', null)` is sinds de biconditionele CHECK geen stand meer die je kúnt opslaan. Vertrekt de eigenaar, dan sluit de taak (hierboven). En een gearchiveerde groep blijft lézen (`mag_groep_lezen()`) terwijl je er niets meer mee kunt délen (`is_group_member()`); dat is de lees/schrijf-splitsing van 0153, en `archiefleesgat()` werd rood toen de policy die eerst niet volgde. ⚠️ Wat een blokkade betreft gedraagt dit oppervlak zich als alle andere: `blokkade_met_groep()` is een toetredingstoets en geen leesfilter, dus twee groepsgenoten die elkaar geblokkeerd hebben blijven elkaars gedeelde taken lezen — net als elkaars chat en weekdoelen. Systemisch, staat als rij in `docs/ENGINEER-REVIEW.md`. Getest in `tests/rls/taak-delen.test.ts` (vijftien grendels, elk apart met de hand rood gemaakt) |
...
| 32 | Een gedeelde taak uit De Lijst | Niets. Wat dit oppervlak opent hangt aan het **vinkje van de eigenaar** en niet aan `groups.zichtbaarheid`: hij wijst per taak één groep aan, en die groep ziet in beide standen hetzelfde. ⚠️ Een **open** groep krijgt hier dus níét meer dan een beschermde, en dat is met opzet: de grens die telt is *heeft de eigenaar deze regel aangewezen*, en die vraag heeft met de groepsstand niets te maken. ⚠️ Wie dit ooit aan A41 wil koppelen — bijvoorbeeld "in een open groep zie je de hele lijst" — bouwt daarmee variant C alsnog, en die is op 09-09-2026 met zoveel woorden afgewezen | ✅ n.v.t. |
