...
| 18 | **QS8-416 — `copy` is een INSERT** | Uit de audit van 10-09. 0239 trok elk UPDATE-pad op `storage.objects` in en schreef erbij dat daarmee ook het move/copy-eindpunt dicht was; dat dekt `move` en niet `copy`, want een `copy` schrijft een **nieuwe rij**. Dezelfde zin stond in 0235, 0240 en twee dossierrijen | ✅ **af op 11-09**, migratie `0254` — en hij doet mínder dan het issue aannam. 📏 De securityronde vond dat de extensieregex géén enkele kruisrichting sluit: bij een `copy` kiest de client de **doelnaam** (`sourceKey` en `destinationKey` zijn twee losse parameters in `@supabase/storage-js` 2.112.3), dus een pdf uit `chatdocs` heet in `chatfotos` gewoon `onschuldig.jpg`. Zelf nagemeten als `authenticated` ná 0254: alle vier de emmers laten een vrij gekozen doelnaam door. **Wat 0254 wél oplevert:** de naamvorm is in alle vier de emmers een databaseeigenschap in plaats van een clienteigenschap — dezelfde belofte die 0240 voor `chatdocs` waarmaakte, met `evil.html` als meting. De open copy-route staat op **Middel** in `docs/ENGINEER-REVIEW.md`, met `chatdocs` → fotoemmer als zwaarste richting (5 MB tegen 1 MB): op databaseniveau niet te sluiten, want een INSERT-policy ziet alleen de nieuwe rij. ⚠️ De bijbehorende test heette naar `0237` terwijl de drops in `0239` staan — zes keer, inclusief de naam van het `describe`. Uitleg in `docs/decisions/2026-09-11-een-copy-is-geen-move.md` |
