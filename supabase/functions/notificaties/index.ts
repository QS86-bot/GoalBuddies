 ...
 *    bevestigd*. Zie `_shared/notificaties/regels.ts` en migratie 0174 voor de
...
async function draaiNotificaties(auth: string): Promise<Response> {
  ...
  for (const profiel of (profielen ?? []) as Profiel[]) {
    ...
    //    migratie 0174 en in `docs/decisions/2026-09-07-de-getuige-hoort-het-...`.
    //
 ...
 *
 * ⚠️ Via `getuigenissen_voor()` (migratie 0174) en **niet** via `getuigenissen()`.
