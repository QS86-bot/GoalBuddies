-- 0125_een_avatarpad_wijst_naar_jezelf.sql — avatar_url is een pad in je eigen map, of niets
--
-- ROLLBACK-PAD:
...
-- ---------------------------------------------------------------------------
--
-- 📏 Gemeten op 28-08-2026, direct na 0124, in `information_schema.column_privileges`:
...
--      ⚠️ De ondertekening van 0124 vangt dit al — wat niet te tekenen valt wordt
...
-- ---------------------------------------------------------------------------
--
-- Dezelfde vorm als de policies van 0124: het eerste padsegment ís de eigenaar.
...
--    `src/` en `app/` vóór 0124). `not valid` is daarom niet nodig.
--
-- ---------------------------------------------------------------------------
...
comment on constraint profiles_avatar_url_eigen_pad on public.profiles is
  'Migratie 0125. avatar_url draagt sinds 0124 een pad in de privébucket avatars, '
