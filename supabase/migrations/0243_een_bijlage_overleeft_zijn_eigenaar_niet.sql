-- 0243_een_bijlage_overleeft_zijn_eigenaar_niet.sql — het AVG-verwijderpad kent
...
--      om op de CHECK van 0242. Ruim eerst `chatdocs` op, of draai 0242 mee
--      terug.
--
-- ---------------------------------------------------------------------------
...
-- document plaatste.
--
-- ⚠️⚠️ **En er is een tweede reden die pas met 0242 ontstond, en die is
...
-- `doc`-rij zonder bijlage niet op de naam-CHECK van 0242 zou omvallen.
--
...
-- is precies het veld dat bepaalt tegen welke emmer de client tekent (0242 §1),
-- dus het is het laatste veld dat je los zou moeten maken om een opruiming te
-- laten slagen.
--
-- De reparatie zit daarom in 0242: de naam-eis hangt aan de **bijlage** en niet
...
begin
  ...
  --    dat hoort zo. De naam-CHECK van 0242 hangt daarom aan de bijlage. Zie §3.
...
--    `set null` uitvoert. Erna zou de CHECK van 0242 al omgevallen zijn.
