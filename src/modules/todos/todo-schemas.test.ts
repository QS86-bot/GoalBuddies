 ...
 *   N  `TAAK_MAX` op 400
 *      -> 1 rood: 'TAAK_MIN en TAAK_MAX komen letterlijk uit de CHECK van 0219'
 ...
 *      -> 1 rood: 'VOLGORDE_MAX komt letterlijk uit de CHECK van 0219'
...
const MIGRATIE = readFileSync(
  'supabase/migrations/0219_de_lijst_krijgt_een_tabel_die_dicht_staat.sql',
  'utf8',
);
...
describe('de grenzen staan in de database én in het schema, en ze zijn gelijk', () => {
  it('TAAK_MIN en TAAK_MAX komen letterlijk uit de CHECK van 0219', () => {
    const uitMigratie =
      /char_length\(btrim\(body\)\)\s*between\s*(\d+)\s*and\s*(\d+)/i.exec(MIGRATIE);
    expect(uitMigratie, 'geen todo_items_body_len in 0219').not.toBeNull();
   ...
   */
  it('VOLGORDE_MAX komt letterlijk uit de CHECK van 0219', () => {
    const uitMigratie = /order_index\s+between\s+(\d+)\s+and\s+(\d+)/i.exec(MIGRATIE);
    expect(uitMigratie, 'geen todo_items_order_bereik in 0219').not.toBeNull();
...
describe('het schema biedt geen pad dat de database niet heeft', () => {
   ...
   *    enkele client schrijfbaar (0219, kolomgrant én policy). Zou hij hier wél
  ...
  /**
   * ⚠️ De drie velden hieronder zijn precies de UPDATE-kolomgrant van 0219. Komt
   ...
   */
  it('kent in een patch precies de kolommen uit de UPDATE-grant van 0219', () => {
    const uitMigratie = /grant update \(([^)]+)\)\s+on table public\.todo_items/i.exec(MIGRATIE);
    expect(uitMigratie, 'geen UPDATE-kolomgrant in 0219').not.toBeNull();
