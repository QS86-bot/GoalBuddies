 ...
 *    2. 5 MB in plaats van 1 MB (byte-pariteit per groep — zie 0238 §4);
 ...
 *
 * ⚠️⚠️ **Een kopie van `allowed_mime_types` in migratie 0238 — en dáár is de
...
/** De grens die 0238 op de bucket zet: 5 MB. */
export const CHATDOC_MAX_BYTES = 5_242_880;

/** De grens die de CHECK van 0240 op de naam zet, in **codepunten**. */
 ...
 *    (de CHECK van 0240 laat er alleen `[A-Za-z0-9._-]` in toe).
 *
 ...
 *    vertelt. De CHECK van 0240 weigert zo'n naam; deze functie zorgt dat de
...
export function schoneBestandsnaam(ruw: string): string {
  const zonderRegie = ruw
    ...
    //    (0240) weigert — één klasse, twee plekken, en die twee horen gelijk te
 ...
 *
 * ⚠️ Het pad ligt vast in de CHECK van 0240; de naam is gebruikerstekst. Zou de
...
export async function uploadChatdoc(
  ...
  if (gezet.error) {
    ...
    // ⚠️ Het dagplafond van 0239 komt hier ook binnen, en dat is te onderscheiden
