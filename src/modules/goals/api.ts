...
/**
 * Wat een client van een `goals`-rij terugkríjgt sinds migratie 0232.
 *
 * ⚠️⚠️ **`Doel` spiegelt de kolommen van de tabel; dit type spiegelt de
 *    kolomgrant.** Die twee liepen uiteen op de dag dat 0232 vijf kolommen
 ...
 *
 * ⚠️⚠️ **Ze staan sinds migratie 0232 niet meer op `goals` voor `authenticated`**,
...
export async function maakDoel(
  ...
  //    gerepareerd defect.** Sinds migratie 0232 heeft `authenticated` geen
...
export async function wijzigDoel(
  ...
  // ⚠️ Zelfde reden als bij `maakDoel()`: sinds 0232 is `*` op `goals` een
