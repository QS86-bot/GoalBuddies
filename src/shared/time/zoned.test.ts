...
import { apparaatTijdzone, isGeldigeTijdzone, localDateIn, utcFromZoned } from './zoned';

/**
 * (De tests van `klokTijd` zijn op 03-09-2026 mee verhuisd naar
 * `opmaak.test.ts`, samen met de functie zelf — QS8-221.)
 *
 * `toonKlokTijd` staat in `shared/time` en niet bij de chat, omdat het een
...
describe('apparaatTijdzone', () => {
  it('geeft een zone die de rest van deze module aankan', () => {
    ...
    expect(localDateIn(zone, new Date('2026-08-24T12:00:00Z'))).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });
